...
public class CompetitorDAOTest {
	...
	private static final String IMPORT_FILE_NAME = "competitor.csv";
	...
	public void shouldCreateNewObjectInEmbeddedMongoDb() throws IOException {
		...
		try {
			isImported = service.importCompetitorFile(content, IMPORT_FILE_NAME,
					datastore);
		...
		long totalMatchedCount = 0;
		try {
			totalMatchedCount = service.getTotalMatchedProductCount(datastore);
			Assert.assertEquals(totalMatchedCount, 2764);
		} catch (InvalidParameterException | SystemException e) {
			Assert.assertEquals(totalMatchedCount, 0);
		}
